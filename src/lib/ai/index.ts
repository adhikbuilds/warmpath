...
class AzureAIProvider implements AIProvider {
  ...
  private buildRequestBody(input: GenerateMessageInput): Record<string, unknown> {
    const {
      account,
      contact,
      signal,
    ...
    const body: Record<string, unknown> = {
      ...
      account_employee_count: account.employee_count ?? undefined,
      ...
      kb_items: kbItems
        .filter((k) => k.approved_for_ai)
        .slice(0, 8)
        .map((k) => ({
          type: k.type,
          title: k.title,
          content: k.content,
          approved_for_ai: k.approved_for_ai,
        })),
    };

    if (signal) {
      body.signal_type = signal.type;
      body.signal_title = signal.title;
      body.signal_description = signal.description ?? undefined;
    }

    if (warmPath) {
      body.warm_path = warmPath.path_nodes.map((n) => n.name);
      body.intro_person = warmPath.recommended_intro_person ?? undefined;
    }
  ...
  async generateMessage(input: GenerateMessageInput): Promise<GeneratedMessageResult> {
    ...
    const { channel = "email", kbItems = [] } = input;

    let res: Response;
    try {
      res = await fetch("/api/ai/azure", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.error("[AzureAI] Fetch error, falling back to mock:", err);
      return new MockAIProvider().generateMessage(input);
    }

    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      console.error("[AzureAI] Route error, falling back to mock:", err);
      return new MockAIProvider().generateMessage(input);
    }
    ...
    const result: GeneratedMessageResult = {
      ...
      intro_request: typeof data.intro_request === "string" ? data.intro_request : undefined,
      channel,
      confidence_score:
        typeof data.confidence_score === "number"
          ? Math.max(0.75, Math.min(0.97, data.confidence_score))
          : 0.86,
      personalization_reason:
        typeof data.personalization_reason === "string"
          ? data.personalization_reason
          : `Generated by Azure OpenAI (${AZURE_DEPLOYMENT})`,
      factual_claims: Array.isArray(data.factual_claims) ? (data.factual_claims as string[]) : [],
      supporting_sources: [`Azure OpenAI: ${AZURE_DEPLOYMENT}`],
      risk_flags: Array.isArray(data.risk_flags) ? (data.risk_flags as string[]) : [],
      used_kb_item_ids: kbItems.filter((k) => k.approved_for_ai).map((k) => k.id),
    };
  ...
  async analyzeRisk(body: string, subject?: string): Promise<MessageRisk[]> {
    return new MockAIProvider().analyzeRisk(body, subject);
  }
...
export function getAIProvider(): AIProvider {
  ...
  if (mode === "azure") {
    providerInstance = new AzureAIProvider();
  } else if (mode === "remote") {
...
export function getAIMode(): AIMode {
  // Explicit user override in localStorage always wins
  if (typeof window !== "undefined") {
    const stored = localStorage.getItem("warmpath-ai-mode");
    if (stored === "mock" || stored === "local" || stored === "remote" || stored === "azure")
      return stored;
  }

  // Env var override (set in .env.local for dev or CI)
  const envMode = process.env.NEXT_PUBLIC_AI_MODE as AIMode | undefined;
  if (envMode && ["mock", "local", "remote", "azure"].includes(envMode)) return envMode;

  // In demo mode (NEXT_PUBLIC_DEMO_MODE=true) default to mock — no credits burned during demos
  // Real workspace: default to azure (server-side key, never exposed to browser)
