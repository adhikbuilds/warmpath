...
export default function CampaignBuilderPage() {
  ...
  const handleLaunch = useCallback(async () => {
    ...
    const campaignName = `${GOALS.find((g) => g.id === config.goal)?.label ?? "Campaign"} — ${config.industries.join(", ") || "All Industries"}`;

    // Persist to DB before proceeding
    ...
    try {
      const res = await fetch("/api/campaigns", {
        ...
        body: JSON.stringify({
          ...
          goal: config.goal,
          status: "active",
          steps: config.sequence.map((s) => ({
            channel: s.channel,
            delayDays: s.day,
            actionType: s.objective,
          })),
        }),
      });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast.error(err.error ?? "Failed to save campaign. Please try again.");
        setLaunching(false);
        return;
      }
      ...
      toast.error("Network error — could not save campaign. Please try again.");
      setLaunching(false);
      return;
    }
